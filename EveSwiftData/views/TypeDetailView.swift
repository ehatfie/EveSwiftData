...
//    )
    ...
    let attributeModels = data.map { (typeDogmaAttributeInfoModel, attributeModel) in
      AttributeValue(
    ...
    let attributeCategoryValues = attributeCategories.map { key, value in
      return AttributeCategoryValue(categoryId: key, attributeValues: value.sorted(by: { $0.attributeId < $1.attributeId}))
    }
    self.attributeCategories = attributeCategoryValues.sorted(by: { $0.categoryId < $1.categoryId })
  }
...
struct TypeDetailView: View {
  ...
  init(typeId: Int64, modelContext: ModelContext) {
    self.viewModel = TypeDetailViewModel(typeId: typeId, modelContext: modelContext)
    self._typeModels = Query(
      filter: TypeModel.predicate(typeId: typeId)
    )
  ...
  var body: some View {
    VStack(alignment: .leading) {
      ...
      Grid {
        ...
        if let foo = viewModel.categorizedAttributes[37] {
          GridRow {
            DogmaBonusView(dogmaAttributes: foo)
          }
        }
            ...
            Grid(alignment: .leading) {
              ForEach(attributeCategory.attributeValues) { attributeValue in
                GridRow {
                  ...
                  Text(attributeValue.text + " \(attributeValue.attributeId)")
