...
class FitMathHelperViewModel {
  ...
  func setupSelectedItem() {
    guard let selectedItemID,
      let itemDogmaInfo = makeItemDogmaInfo(typeId: selectedItemID)
    else {
      return
    }
  ...
  func applyModifiers(
    ...
    for modifier in modifiers {
      ...
      guard
        let modifyingAttributeValue = sourceAttributes[modifyingAttributeID],
        let modifiedAttributeValue = attributes[modifiedAttributeID]
      else {
      ...
      guard
        let dogmaModifierOperation = DogmaModifierOperation(rawValue: operation)
      else {
        continue
      }

      print(
        "++ domain \(domain) function \(function) operation \(dogmaModifierOperation)"
      )
      ...
      print(
        "++ modifies \(modifiedAttributeValue.value) by \(modifyingAttributeValue.value) to \(newValue)"
      )
...
struct FitMathHelperView: View {
  ...
  var body: some View {
    HStack(alignment: .top) {
      ...
      List {
        VStack {
          TypeInfo(typeId: viewModel.defaultTypeId)
          AttributesDetailView(
            categorizedDogmaAttributes: $viewModel
              .modifiedCategorizedDogmaAttributes,
            defaultCategorizedDogmaAttributes: viewModel
              .categorizedDogmaAttributes,
          )
        }
        //Text("AttributesDetailView")
        // Add damage View
...
extension FitMathHelperViewModel {
  ...
  func makeEffectValues(for dogmaEffects: [TypeDogmaEffectInfoModel])
    -> [DogmaEffectValue]
  {
    return dogmaEffects.compactMap(makeDogmaEffectValue(for:))
  }

  func makeDogmaEffectValue(for typeDogmaEffectInfo: TypeDogmaEffectInfoModel)
    -> DogmaEffectValue?
  {
    guard
      let dogmaModel = getDogmaEffectModel(for: typeDogmaEffectInfo.effectID)
    else {
      return nil
    }

    return DogmaEffectValue(
      typeDogmaEffectInfoModel: typeDogmaEffectInfo,
      dogmaEffectModel: dogmaModel
    )
  }

  func makeAttributeValues(for attributes: [TypeDogmaAttributeInfoModel])
    -> [AttributeValue]
  {
  ...
  func makeAttributeValue(
    for typeDogmaAttributeInfoModel: TypeDogmaAttributeInfoModel
  ) -> AttributeValue? {
    let attributeID = typeDogmaAttributeInfoModel.attributeID
    guard let dogmaAttributeModel = self.getAttributeModel(for: attributeID)
    else {
      return nil
    }
  ...
  func processAttributes(dogmaAttributeModels: [AttributeValue]) {
    let categorizedAttributes = self.makeCategorizedAttributes(
      for: dogmaAttributeModels
    )
